...
InitiationDispatcher::~InitiationDispatcher() {
    for (std::map<int, EventHandler *>::iterator it = handlers_.begin(); it != handlers_.end();
         ++it) {
        delete it->second;
    }
    handlers_.clear();
...
void InitiationDispatcher::registerHandler(EventHandler *handler) {
...
void InitiationDispatcher::handleEvents() {
    ...
    while (!epollManager_.shouldShutdown()) {
